<template>
    <div class="mylibrary" v-if="this.radios==0">
        ...
        <div class="mylibrary-informat">
            ...
            <template>
                <div class="index7_4" style="overflow: hidden;">
                    <ul>
                        <li>
                            <div class="aboutme_position" :data="data1">
                                <i class="el-icon-add-location"></i>当前位置 > 我的图书馆 > 我的书架
                            </div>
                            <div class="about_index7_4" style="float:left">
                                <div class="listname4">
                                    <div >
                                    ...
                                    <div>
                                        <a href="/mybookshelf"><el-button style="color: #CD3339;"><i class="el-icon-arrow-right"></i> 我的书架</el-button></a>
                                    </div>
                                    <div>
                                ...
                                <div class="listbook4">
                                    <el-table
                                        ...
                                        <template slot-scope="scope">
                                            <el-button @click="getDetails(scope.$index, scope.row)" type="text" size="small">详情</el-button>
                                            <el-button type="text" size="small" @click="handleDelete(scope.$index, scope.row)">移除</el-button>
                                        </template>
                                        </el-table-column>
                                        <!-- <el-table-column
                                        fixed="right"
...
export default {
    ...
    methods:{
        mybookshelf:async function(){
            if(this.radios==0){
                let self = this;
                let {status,data:{list}} = await self.$axios.post('/mylibrary/mybookshelf', 
        ...
        handleDelete(index, row) {
            ...
            this.$confirm('从我的书架移除'+' '+row.book_name+'， '+'点击“确定”移除。' , '请确认以下信息', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                }).then(() => {
                    this.state(row);
         ...
         state:async function(row){
             console.log( row);
            let self = this;
            let {status,data:{list}} = await self.$axios.post('/mylibrary/cancel_collect_book',
            {book_id:row.book_id,user_name:self.username})
        ...
        getDetails:async function(index, row){
