<template>
    <div class="returnbooks" v-if="this.radios==1">
        <account/>
        <div class="seven">
            <div class="nav">
                <ul>
        ...
        <div class="returnbooks-body">
            <returnbooks :listb='listb'/>
            <!-- <buttom/> -->
        </div>
    </div>
</template>
...
import returnbooks from '../components/administrators/return.vue'
...
export default {
    ...
    async asyncData(ctx) {
        ...
        try{
            const {data:{list}} = await ctx.$axios.post('/borrow/borrowuser')
