<template>
    <div class="administrators" v-if="this.radios==1">
        <account/>
        <div class="seven">
            <div class="nav">
                <ul>
        ...
        <div class="administrators-body">
            <administratorrights :listb="listb"/>
            <!-- <buttom/> -->
        </div>
    </div>
</template>
...
import administratorrights from '../components/administrators/administratorrights.vue'
import { async } from 'q';
...
export default {
    ...
    async asyncData(ctx) {
        try{
            const {data:{list}} = await ctx.$axios.post('/administrators/eachars')
    ...
    methods:{
        logonstatus:async function(){
            // let self = this;
            // let {status,data:{list}} = await self.$axios.post('/administrators/eachars')
            // self.listb=list.data
