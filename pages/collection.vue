<template>
    <div class="collection" v-if="this.radios==1">
        <account/>
        <div class="seven">
            <div class="nav">
                <ul>
...
import collectionbooks from '../components/administrators/collectionbooks.vue'
export default {
    components: {
        account,
        collectionbooks,
    ...
    async asyncData(ctx) {
        try{
            const {data:{list}} = await ctx.$axios.post('/collection/booklist')
