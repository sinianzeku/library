<template>
    <div class="lentout" v-if="this.radios==1">
        <account/>
        <div class="seven">
            <div class="nav">
                <ul>
...
import lend from '../components/administrators/lend.vue'
...
export default {
    ...
    async asyncData(ctx) {
        try{
            const {data:{list}} = await ctx.$axios.post('/lentout/booklist')
