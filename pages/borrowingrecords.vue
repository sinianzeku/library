<template>
    <div class="borrowingrecords" v-if="this.radios==1">
        <account/>
        <div class="seven">
            <div class="nav">
                <ul>
        ...
        <div class="borrowingrecords-body">
            <borrowing :listb="listb"/>
        </div>
    </div>
</template>
...
import borrowing from '../components/administrators/borrowing.vue'
export default {
    components: {
        account,
        borrowing,
    ...
    async asyncData(ctx) {
        try{
            const {data:{list}} = await ctx.$axios.post('/borrowingrecords/lendbooklist')
